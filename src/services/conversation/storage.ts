...
export interface ConversationPreferences {
  ...
  autoCompact: boolean;
  useMemories: boolean;
  /** Separate opt-in; never enabled by migrating an existing preference. */
  automaticMemories?: boolean;
...
export type MemoryScope = {kind: 'shared'} | {kind: 'pal'; palId: string};
...
export async function savePalMemoryIsolation(
  palId: string,
  enabled: boolean,
): Promise<void> {
  await AsyncStorage.setItem(
    palIsolationKey(palId),
    JSON.stringify(enabled === true),
  );
  conversationRuntime.preferencesChanged();
}
...
export function normalizePreferences(value: unknown): ConversationPreferences {
  ...
  return {
    ...
    automaticMemories: raw.automaticMemories === true,
    thinkingBudget:
      typeof raw.thinkingBudget === 'number' &&
      Number.isSafeInteger(raw.thinkingBudget) &&
      raw.thinkingBudget >= 0 &&
      raw.thinkingBudget <= 1000000
        ? raw.thinkingBudget
        : undefined,
  };
}
...
export async function readPreferences(
  ...
  try {
    ...
    throw new Error(
      'Conversation settings could not be read. No settings were changed.',
    );
  }
}
...
export class MobileMemoryRepository {
  ...
  eraseAll(
    ...
    return this.serialize(scope, async () => {
      await authorize?.();
      await AsyncStorage.removeItem(memoryKey(scope));
    });
  }
}
...
class ConversationRuntimeStore {
  ...
  setStatus(sessionId: string, message: string) {
    runInAction(() => {
      this.statuses[sessionId] = message;
    });
  }
}
